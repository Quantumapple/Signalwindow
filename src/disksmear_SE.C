...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      //int genN = tmPpropgenElPartEta->size();
      //int bN = tmPbRecHitGx->size(); 
      //int fN = tmPfRecHitGx->size(); 
      //int EgN = tmPegCrysClusterEt->size();

      //cout << "evnet : " << jentry << endl;
      //cout << "genN : " << genN << ", bN : " << bN << ", fN : " << fN << ", EgN : " << EgN << endl; 
      //cout << "=========================================================" << endl; 
      ...
      float gen_Eta = tmPpropgenElPartEta->at(0);
      int eta_region = 0;
      if( fabs(gen_Eta) <= 0.8 ) eta_region =1; 
      if( fabs(gen_Eta) <= 1.4 && fabs(gen_Eta) > 0.8 ) eta_region =2;
      if( fabs(gen_Eta) <= 1.7 && fabs(gen_Eta) > 1.4 ) eta_region =3;
      if( fabs(gen_Eta) <= 2.1 && fabs(gen_Eta) > 1.7 ) eta_region =4;
      if( fabs(gen_Eta) <= 2.7 && fabs(gen_Eta) > 2.1 ) eta_region =5;
      if( fabs(gen_Eta) <= 3.0 && fabs(gen_Eta) > 2.7 ) eta_region =6;

      // x_smear = y_smear
      float x_smear = 0.00072;

      float z1_smear = 0.001;
      float z2_smear = 0.001;
      float z3_smear = 0.001;
      float z4_smear = 0.001;
      float z5_smear = 0.001;
      ...
      for(int i=0; i < tmPfRecHitGx->size(); i++){
          ...
          //fRecHitGx.push_back(fx);
          //fRecHitGy.push_back(fy);
          //fRecHitGz.push_back(fz);

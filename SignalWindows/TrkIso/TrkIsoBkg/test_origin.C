        ...
        if( EgN <= 6 )
        {
            ...
            for( int q=0; q<EgN; q++){ 
                ...
                if( all.size() == 0 ) { 
                    ...
                    if( vec_size >= 16 ) vec_size = 15;
                    //cout << "    pT list" << endl;
                    //for(Int_t k = 0; k < vec_size; k++) cout << "     pT: " << pT_vector.at(k) << endl;
                    for(Int_t k = 0; k < vec_size; k++) { denomi += pT_vector[k].pt; h_tracketa_all->Fill(pT_vector[k].eta);} 
                    ...
                    if( eta_region == 1 || eta_region == 2 ) {
                        if( ratio < 0.09 ) ntCl_iso_match.push_back(true);
                        else ntCl_iso_match.push_back(false);
                    }
                    if( eta_region == 3 ) {
                        if( ratio < 0.04 ) ntCl_iso_match.push_back(true);
                        else ntCl_iso_match.push_back(false);
                    }
                    if( eta_region == 4 ) {
                    ...
                    if( eta_region == 5 ) {
                        if( ratio < 0.41 ) ntCl_iso_match.push_back(true);
                        else ntCl_iso_match.push_back(false);
                    }
                    if( eta_region == 6 ) {
                        if( ratio < 0.35 ) ntCl_iso_match.push_back(true);
                        else ntCl_iso_match.push_back(false);
                    }
                }
                ...
                if( all.size() == 0 ) { 
                    ...
                    for(Int_t cur = 0; cur < all_size; cur++) {
                        if( all[cur].index == 1 ) {
                            ...
                            Float_t recopT = pT_fit(eta_region, all[cur].index, dPhi);
                            Float_t trackEta = pixel2.Eta();
                            pT_vector.push_back(leading(recopT, trackEta));
                        }
                        if( all[cur].index == 2 ) {
                            TVector3 pixel1; pixel1.SetXYZ( all[cur].pos_x2, all[cur].pos_y2, all[cur].pos_z2 - recoPV );
                            TVector3 pixel2; pixel2.SetXYZ( all[cur].pos_x3 - all[cur].pos_x2, all[cur].pos_y3 - all[cur].pos_y2, all[cur].pos_z3 - all[cur].pos_z2 );

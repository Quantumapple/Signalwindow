...
def now():
    ...
    return str(datetime.now().month) + str(datetime.now().day)+ str(datetime.now().hour)+str(datetime.now().minute)+str(datetime.now().second)

sample = "Bkg_200PU"

InputDir = "/xrootd/store/user/jongho/Delphes/newBkg/"+str(sys.argv[1])
#InputDir ="/xrootd/store/user/jongho/Delphes/hmanBkg/" ### Hokyeong samples
...
for name in lines:
    newFile.write("root://cms-xrdr.private.lo:2094///xrd/store/user/jongho/Delphes/newBkg/" + sys.argv[1] + "/" + name)
    #newFile.write("root://cms-xrdr.private.lo:2094///xrd/store/user/jongho/Delphes/hmanBkg/" + name) ### Hokyeong samples
    line_count += 1
...
number_of_files = line_count

print "Job has " + str(number_of_files) + " files to process:"

number_of_cores = line_count/4
...
###################################################
workspace = "/cms/ldap_home/jongho/TrkIso/TrkIsoBkg/Results/"
